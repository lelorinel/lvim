...
return {
  ...
  -- 1. Yükleme Tetikleyicisi: 'ft' yerine 'LspAttach' kullanmak daha verimlidir.
  -- Bu sayede Lspsaga, sadece bir dil sunucusu gerçekten başladığında yüklenir.
  event = "LspAttach",
  -- 2. Bağımlılıklar: İkonlar ve bazı özellikler için gerekli eklentiler.
  dependencies = {
    "nvim-tree/nvim-web-devicons",
    "nvim-lua/plenary.nvim", -- Bazen arka planda gerekebilir.
  },
  -- 3. Tuş Atamaları: Mevcut tuş atamalarınız korunmuştur.
  keys = {
    ...
    { "gD", "<cmd>Lspsaga goto_definition<CR>", desc = "Lspsaga: Goto Definition" }, -- Genellikle 'gd' kullanılır, sizinkini korudum.
    { "gj", "<cmd>Lspsaga diagnostic_jump_next<CR>", desc = "Lspsaga: Next Diagnostic" },
    { "gk", "<cmd>Lspsaga diagnostic_jump_prev<CR>", desc = "Lspsaga: Prev Diagnostic" },
    {
      "gJ",
      function()
        require("lspsaga.diagnostic").goto_next({ severity = vim.diagnostic.severity.ERROR })
      end,
      desc = "Lspsaga: Next Error",
    },
    {
      "gK",
      function()
  ...
  -- 4. Yapılandırma: 'opts' kullanarak daha temiz bir yapı.
  opts = {
    ui = {
      border = "rounded",
      title = true,
      ...
      collapse = "",
      code_action = "󰛨",
      -- kind = {}, -- Nerd Font ikonlarını otomatik olarak alır, genellikle boş bırakmak daha iyidir.
    },
    diagnostic = {
      show_code_action = true,
      show_source = true,
      jump_num_shortcut = true,
      text_hl_follow = false, -- Performans için false kalması önerilir.
      keys = { exec_action = "o", quit = "q", go_action = "g" },
    },
    lightbulb = {
      enable = true,
      ...
      sign = true,
      sign_priority = 40,
      virtual_text = false, -- En iyi görünüm için bu false olmalı.
    },
    finder = {
      keys = {
        shuttle = "[w",
    ...
    rename = {
      in_select = true, -- Seçim modunda yeniden adlandırma.
      keys = {
        quit = "<C-c>",
        exec = "<CR>",
      },
    ...
    symbol_in_winbar = {
      enable = false, -- Genellikle lualine gibi eklentilerle çakışmaması için kapalı tutulur.
    },
  },
}
